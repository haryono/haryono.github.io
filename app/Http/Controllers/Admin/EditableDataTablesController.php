...
class EditableDataTablesController extends Controller
{
    ...
    {
        $tables = Companyprofile::get(['id','company_name', 'company_initial','company_reg_no']);
    ...
    {
        if($request->ajax()) {
            Companyprofile::create($request->except('_token'));
        }
    }
    ...
    {
        $tables=Companyprofile::get(['id', 'company_name', 'company_initial', 'company_reg_no', 'status']);
        return Datatables::of($tables)
            ->addColumn('edit', '<a class="edit" href="javascript:;">Edit</a>')
            ->addColumn('delete', '<a class="delete" href="#" data-target="#deleteConfirmModal" data-toggle="modal">Delete</a>')
